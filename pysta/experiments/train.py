...
# Check if DGL supports CUDA (not just PyTorch)
def check_dgl_cuda():
    try:
        import dgl
        # Try to create a small graph on CUDA
        g = dgl.graph(([0], [1]))
        g = g.to('cuda')
        return True
    except Exception:
...
USE_CUDA = check_dgl_cuda()
if USE_CUDA:
    model.cuda()
    print("Using CUDA for model and graphs")
else:
    print("DGL CUDA not available, using CPU for model and graphs")
    ...
    with torch.no_grad():
        def test_dict(data):
            ...
            for k, g in data.items():
                ...
                r2_sum = r2_sum + r2
            print("DEBUG_r2_avg", r2_sum/args.test_data_number)
        ...
        print("DEBUG_test_time", test_end_time-test_start_time)
